...
**Expected Output:** Aggregated metrics grouped by day and created_by
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.created_by) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
    WHERE c1.rn <= 60
),
cte_level_3 AS (
    SELECT
        c2.*,
        LAG(LENGTH(c2.id), 1) OVER (PARTITION BY c2.created_by ORDER BY c2.created_at) AS prev_value,
        LEAD(LENGTH(c2.id), 1) OVER (PARTITION BY c2.created_by ORDER BY c2.created_at) AS next_value,
        LENGTH(c2.id) - LAG(LENGTH(c2.id), 1) OVER (PARTITION BY c2.created_by ORDER BY c2.created_at) AS delta_value,
        AVG(LENGTH(c2.id)) OVER (PARTITION BY c2.created_by) AS partition_avg,
        STDDEV(LENGTH(c2.id)) OVER (PARTITION BY c2.created_by) AS partition_stddev,
        NTILE(5) OVER (PARTITION BY c2.created_by ORDER BY LENGTH(c2.id)) AS ntile_bucket,
        RANK() OVER (PARTITION BY c2.day_bucket ORDER BY LENGTH(c2.id) DESC) AS daily_rank
    FROM cte_level_2 c2
),
cte_level_4 AS (
    SELECT
        c3.*,
        CASE 
            WHEN c3.partition_stddev > 0 THEN (LENGTH(c3.id) - c3.partition_avg) / c3.partition_stddev
            ELSE 0 
        END AS z_score,
        DENSE_RANK() OVER (ORDER BY c3.cumulative_sum DESC) AS overall_rank,
        PERCENT_RANK() OVER (PARTITION BY c3.created_by ORDER BY LENGTH(c3.id)) AS pct_rank,
        CASE
...
SELECT
    ...
    AVG(LENGTH(c4.id)) AS avg_value,
    PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY LENGTH(c4.id)) AS q1_value,
    PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY LENGTH(c4.id)) AS median_value,
    PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY LENGTH(c4.id)) AS q3_value,
    STDDEV(LENGTH(c4.id)) AS stddev_value,
    MIN(LENGTH(c4.id)) AS min_value,
    MAX(LENGTH(c4.id)) AS max_value,
...
FROM cte_level_4 c4
GROUP BY DATE_TRUNC('day', c4.created_at), c4.created_by
HAVING COUNT(*) >= 2
...
**Expected Output:** Aggregated metrics grouped by week and id
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.id) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN 5 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
    WHERE c1.rn <= 70
),
cte_level_3 AS (
    SELECT
        c2.*,
        LAG(LENGTH(c2.id), 1) OVER (PARTITION BY c2.id ORDER BY c2.created_at) AS prev_value,
        LEAD(LENGTH(c2.id), 1) OVER (PARTITION BY c2.id ORDER BY c2.created_at) AS next_value,
        LENGTH(c2.id) - LAG(LENGTH(c2.id), 1) OVER (PARTITION BY c2.id ORDER BY c2.created_at) AS delta_value,
        AVG(LENGTH(c2.id)) OVER (PARTITION BY c2.id) AS partition_avg,
        STDDEV(LENGTH(c2.id)) OVER (PARTITION BY c2.id) AS partition_stddev,
        NTILE(6) OVER (PARTITION BY c2.id ORDER BY LENGTH(c2.id)) AS ntile_bucket,
        RANK() OVER (PARTITION BY c2.day_bucket ORDER BY LENGTH(c2.id) DESC) AS daily_rank
    FROM cte_level_2 c2
),
cte_level_4 AS (
    SELECT
        ...
        END AS z_score,
        DENSE_RANK() OVER (ORDER BY c3.cumulative_sum DESC) AS overall_rank,
        PERCENT_RANK() OVER (PARTITION BY c3.id ORDER BY LENGTH(c3.id)) AS pct_rank,
        CASE
            WHEN c3.delta_value > 0 THEN 'Increasing'
            WHEN c3.delta_value < 0 THEN 'Decreasing'
            ELSE 'Stable'
...
SELECT
    ...
    MAX(c4.cumulative_sum) AS max_cumulative
FROM cte_level_4 c4
GROUP BY DATE_TRUNC('week', c4.created_at), c4.id
HAVING COUNT(*) >= 3
ORDER BY period DESC, avg_value DESC
LIMIT 100
```
...
**Expected Output:** Aggregated metrics grouped by month and created_by
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.created_by) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_3 AS (
    SELECT
        ...
        AVG(LENGTH(c2.id)) OVER (PARTITION BY c2.created_by) AS partition_avg,
        STDDEV(LENGTH(c2.id)) OVER (PARTITION BY c2.created_by) AS partition_stddev,
        NTILE(7) OVER (PARTITION BY c2.created_by ORDER BY LENGTH(c2.id)) AS ntile_bucket,
        RANK() OVER (PARTITION BY c2.day_bucket ORDER BY LENGTH(c2.id) DESC) AS daily_rank
    FROM cte_level_2 c2
),
cte_level_4 AS (
...
SELECT
    ...
    MAX(c4.cumulative_sum) AS max_cumulative
FROM cte_level_4 c4
GROUP BY DATE_TRUNC('month', c4.created_at), c4.created_by
HAVING COUNT(*) >= 1
ORDER BY period DESC, avg_value DESC
LIMIT 100
```
...
**Expected Output:** Aggregated metrics grouped by day and id
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.id) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_3 AS (
    SELECT
        ...
        AVG(LENGTH(c2.id)) OVER (PARTITION BY c2.id) AS partition_avg,
        STDDEV(LENGTH(c2.id)) OVER (PARTITION BY c2.id) AS partition_stddev,
        NTILE(8) OVER (PARTITION BY c2.id ORDER BY LENGTH(c2.id)) AS ntile_bucket,
        RANK() OVER (PARTITION BY c2.day_bucket ORDER BY LENGTH(c2.id) DESC) AS daily_rank
    FROM cte_level_2 c2
),
cte_level_4 AS (
...
SELECT
    ...
    MAX(c4.cumulative_sum) AS max_cumulative
FROM cte_level_4 c4
GROUP BY DATE_TRUNC('day', c4.created_at), c4.id
HAVING COUNT(*) >= 2
ORDER BY period DESC, avg_value DESC
LIMIT 100
```
...
**Expected Output:** Aggregated metrics grouped by week and created_by
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.created_by) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN 8 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_3 AS (
    SELECT
        ...
        AVG(LENGTH(c2.id)) OVER (PARTITION BY c2.created_by) AS partition_avg,
        STDDEV(LENGTH(c2.id)) OVER (PARTITION BY c2.created_by) AS partition_stddev,
        NTILE(9) OVER (PARTITION BY c2.created_by ORDER BY LENGTH(c2.id)) AS ntile_bucket,
        RANK() OVER (PARTITION BY c2.day_bucket ORDER BY LENGTH(c2.id) DESC) AS daily_rank
    FROM cte_level_2 c2
),
cte_level_4 AS (
...
SELECT
    ...
    MAX(c4.cumulative_sum) AS max_cumulative
FROM cte_level_4 c4
GROUP BY DATE_TRUNC('week', c4.created_at), c4.created_by
HAVING COUNT(*) >= 3
ORDER BY period DESC, avg_value DESC
LIMIT 100
```
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.id) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN 9 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_3 AS (
    SELECT
        ...
        AVG(LENGTH(c2.id)) OVER (PARTITION BY c2.id) AS partition_avg,
        STDDEV(LENGTH(c2.id)) OVER (PARTITION BY c2.id) AS partition_stddev,
        NTILE(4) OVER (PARTITION BY c2.id ORDER BY LENGTH(c2.id)) AS ntile_bucket,
        RANK() OVER (PARTITION BY c2.day_bucket ORDER BY LENGTH(c2.id) DESC) AS daily_rank
    FROM cte_level_2 c2
),
cte_level_4 AS (
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.created_by) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN 3 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.id) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.created_by) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN 5 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
**Expected Output:** Aggregated metrics grouped by month and id
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.id) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
SELECT
    ...
    MAX(c4.cumulative_sum) AS max_cumulative
FROM cte_level_4 c4
GROUP BY DATE_TRUNC('month', c4.created_at), c4.id
HAVING COUNT(*) >= 2
ORDER BY period DESC, avg_value DESC
LIMIT 100
```
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.created_by) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.id) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN 8 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.created_by) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN 9 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.created_by ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
...
cte_level_2 AS (
    SELECT
        c1.*,
        COUNT(*) OVER (PARTITION BY c1.day_bucket, c1.id) AS daily_partition_count,
        AVG(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN 3 PRECEDING AND CURRENT ROW) AS rolling_avg,
        SUM(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_sum,
        FIRST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at) AS first_val,
        LAST_VALUE(LENGTH(c1.id)) OVER (PARTITION BY c1.id ORDER BY c1.created_at ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS last_val
    FROM cte_level_1 c1
